...
{"theme":"词语类别","normal_word":"普通玩家的词语","undercover_word":"卧底的词语"}`,
    challenge: (playerNames, lives, theme, excludedActions) => `请为"不要做挑战"游戏生成禁忌动作。
...
${excludedActions?.length ? `\n本房间近期已出现动作（本局禁止重复）：${excludedActions.join('、')}` : ''}
...
const CHALLENGE_FALLBACK_ACTIONS = [
    '摸鼻子',
    '摸耳朵',
    '挠头',
    '摸下巴',
...
function sanitizeChallengeAction(action) {
    if (!action)
        return '';
...
function normalizeChallengeAction(action) {
    return sanitizeChallengeAction(action).toLowerCase();
}
...
export class GameRoom {
    ...
    players;
    game = null;
    challengeActionHistory = [];
    challengeActionHistorySet = new Set();
    // AI 配置（房主可修改）
    aiConfig;
    ...
    getRecentChallengeActions(limit = 60) {
        if (limit <= 0)
            return [];
        return this.challengeActionHistory.slice(-limit);
    }
    rememberChallengeActions(actions) {
        for (const action of actions) {
            const clean = sanitizeChallengeAction(action);
            const normalized = normalizeChallengeAction(clean);
            if (!clean || !normalized || this.challengeActionHistorySet.has(normalized)) {
                continue;
            }
            this.challengeActionHistory.push(clean);
            this.challengeActionHistorySet.add(normalized);
        }
        const maxHistory = 300;
        if (this.challengeActionHistory.length > maxHistory) {
            const overflow = this.challengeActionHistory.length - maxHistory;
            const removed = this.challengeActionHistory.splice(0, overflow);
            removed.forEach((action) => {
                const normalized = normalizeChallengeAction(action);
                if (!normalized)
                    return;
                const stillExists = this.challengeActionHistory.some(existing => normalizeChallengeAction(existing) === normalized);
                if (!stillExists) {
                    this.challengeActionHistorySet.delete(normalized);
                }
            });
        }
    }
    ...
    async generateChallengeTasks(playerIds, playerNames, lives, theme) {
        const recentActions = this.getRecentChallengeActions(60);
        const recentActionSet = new Set(recentActions.map(action => normalizeChallengeAction(action)).filter(Boolean));
        const prompt = PROMPTS.challenge(playerNames, lives, theme, recentActions);
        ...
        const allActions = Array.isArray(result.actions) ? result.actions : [];
        const totalActions = playerIds.length * lives;
        const selectedActions = [];
        const selectedActionSet = new Set();
        // 优先使用 AI 返回且不与历史/当前重复的动作
        for (const action of allActions) {
            ...
            if (selectedActionSet.has(normalized) || recentActionSet.has(normalized)) {
                continue;
            }
            selectedActions.push(clean);
            selectedActionSet.add(normalized);
            if (selectedActions.length >= totalActions) {
                break;
            }
        }
        // 不够则从备用池补齐（先避免历史重复）
        if (selectedActions.length < totalActions) {
            console.log(`[GameRoom ${this.code}] 动作数量不足，使用备用动作补齐`);
            const fallbackPool = shuffleArray(CHALLENGE_FALLBACK_ACTIONS);
            for (const fallbackAction of fallbackPool) {
                const normalized = normalizeChallengeAction(fallbackAction);
                if (!normalized) {
                    continue;
                }
                if (selectedActionSet.has(normalized) || recentActionSet.has(normalized)) {
                ...
                selectedActions.push(fallbackAction);
                selectedActionSet.add(normalized);
                if (selectedActions.length >= totalActions) {
                    break;
                }
            }
            // 历史已覆盖过多时，允许复用历史动作，但仍保证本局不重复
            if (selectedActions.length < totalActions) {
                for (const fallbackAction of fallbackPool) {
                    const normalized = normalizeChallengeAction(fallbackAction);
                    if (!normalized || selectedActionSet.has(normalized)) {
                        continue;
                    }
                    selectedActions.push(fallbackAction);
                    selectedActionSet.add(normalized);
        ...
        // 兜底：按序号补齐，避免出现 undefined
        while (selectedActions.length < totalActions) {
            selectedActions.push(`动作${selectedActions.length + 1}`);
        }
        this.rememberChallengeActions(selectedActions);
        // 为每个玩家分配 lives 个动作
        return playerIds.map((id, playerIndex) => {
            const playerActions = [];
            for (let j = 0; j < lives; j++) {
                const actionIndex = playerIndex * lives + j;
                playerActions.push(selectedActions[actionIndex] || '不要笑');
            }
            return {
                playerId: id,
